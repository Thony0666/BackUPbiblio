...
export default function ConfigProf(props) {
    const idProf = props.id;
    const nameProf = props.name;
    const relode = props.function;

    const [openModalDelete, setOpenModalDelete] = React.useState(false);
    const [openModalValidation, setOpenModalValidation] = React.useState(false);
    const [newProf, setProf] = React.useState(nameProf);
    const style = {
        position: 'absolute',
        top: '50%',
        left: '50%',
    ...
    const handleSendProf = () => {
        ...
        axios
            .put(siteUrlApi(`professions`), postData)
            .then((response) => {
                relode(true);
    ...
    const Delete = () => {
        const config = {
            ...
            url: siteUrlApi(`professions/id/${idProf}`),
            headers: {
            ...
            >
                <Box sx={{ ...style, width: 400 }}>
                    ...
                    <p id="parent-modal-description">
                        Vous êtes sur pour efffacer <span style={{ fontWeight: 'bold' }}>{nameProf}</span>
                    </p>
                    <Stack spacing={3} direction={'row'} justifyContent={'flex-end'}>
