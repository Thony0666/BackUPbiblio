...
export default function ConfigRegion(props) {
    const idRegion = props.id;
    const nameRegion = props.name;
    const relode = props.function;

    const [openModalDelete, setOpenModalDelete] = React.useState(false);
    const [openModalValidation, setOpenModalValidation] = React.useState(false);
    const [newRegion, setRegion] = React.useState(nameRegion);
    const style = {
        position: 'absolute',
        top: '50%',
        left: '50%',
    ...
    const handleSendRegion = () => {
        ...
        axios
            .put(siteUrlApi(`regions`), postData)
            .then((response) => {
                relode(true);
    ...
    const Delete = () => {
        const config = {
            ...
            url: siteUrlApi(`regions/id/${idRegion}`),
            headers: {
            ...
            >
                <Box sx={{ ...style, width: 400 }}>
                    ...
                    <p id="parent-modal-description">
                        Vous êtes sur pour efffacer <span style={{ fontWeight: 'bold' }}>{nameRegion}</span>
                    </p>
                    <Stack spacing={3} direction={'row'} justifyContent={'flex-end'}>
