...
export default function ConfigTheme(props) {
    const idTheme = props.id;
    const nameTheme = props.name;
    const relode = props.function;

    const [openModalDelete, setOpenModalDelete] = React.useState(false);
    const [openModalValidation, setOpenModalValidation] = React.useState(false);
    const [newHand, setHand] = React.useState(nameTheme);
    const style = {
        position: 'absolute',
        top: '50%',
        left: '50%',
    ...
    const handleSendRemark = () => {
        ...
        axios
            .put(siteUrlApi(`themes`), postData)
            .then((response) => {
                relode(true);
    ...
    const Delete = () => {
        const config = {
            ...
            url: siteUrlApi(`themes/id/${idTheme}`),
            headers: {
            ...
            >
                <Box sx={{ ...style, width: 400 }}>
                    ...
                    <p id="parent-modal-description">
                        Vous êtes sur pour efffacer <span style={{ fontWeight: 'bold' }}>{nameTheme}</span>
                    </p>
                    <Stack spacing={3} direction={'row'} justifyContent={'flex-end'}>
